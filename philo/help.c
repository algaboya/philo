...
{
	if (mode == UNLOCK)
		return(mtx_error(pthread_mutex_unlock(mtx), UNLOCK), EXIT_FAILURE);
	else if (mode == LOCK)
		return(mtx_error(pthread_mutex_lock(mtx), LOCK), EXIT_FAILURE);
	else if (mode == DESTROY)
		return(mtx_error(pthread_mutex_destroy(mtx), DESTROY), EXIT_FAILURE);
	else if (mode == INIT)
		return(mtx_error(pthread_mutex_init(mtx, NULL), INIT), EXIT_FAILURE);
	else if (mode == DESTROY)
		return(mtx_error(pthread_mutex_destroy(mtx), DESTROY), EXIT_FAILURE);
	else
...
{
	if (mode == CREATE)
		return (thread_error(pthread_create(thread, NULL, abuba, data), CREATE));
	else if (mode == JOIN)
